...
// 换句话说，我们现在开始执行的行为，但它们会在稍后完成。

// 例如，setTimeout 函数就是一个这样的函数。

// 实际中的异步行为的示例，例如加载脚本和模块
...
// 它将带有给定 src 的新动态创建的标签 <script src="…"> 附加到文档中。
// 浏览器将自动开始加载它，并在加载完成后执行。
...
// loadScript('/my/script.js'); // 下面的代码, 不会等到脚本加载完成才执行


// 假设我们需要在新脚本加载后立即使用它。它声明了新函数，我们想运行它们。

// 但如果我们在 loadScript(…) 调用后立即执行此操作，这将不会有效。
    //- loadScript('../js/01.js');  // 这个脚本有 "function newFunction() {…}"
    //- newFunction(); // 没有这个函数！

//- newFunction(); // 01_回调.js:38 Uncaught ReferenceError: newFunction is not defined


// 自然情况下，浏览器可能没有时间加载脚本。到目前为止，loadScript 函数并没有提供跟踪加载完成的方法。
// 脚本加载并最终运行，仅此而已。但我们希望了解脚本何时加载完成，以使用其中的新函数和变量。
...
/*
loadScript2('../js/01.js', function(script) {

    alert(`Cool, the ${script.src} is loaded, let's load one more`);
  
    loadScript('../js/01.1.js', function(script) {
...
/*
loadScript('../js/01.js', function(script) {

    loadScript('../js/01.1.js', function(script) {
  
        loadScript('../js/01.2.js', function(script) {
            // ...加载完所有脚本后继续
        });
...
// 因此，每一个新行为（action）都在回调内部。
// 这对于几个行为来说还好，但对于许多行为来说就不好了，所以我们很快就会看到其他变体。
...
// 用法：
loadScript3('../js/01.js', function(error, script) {
    if (error) {
        // 处理 error
    } else {
        // 脚本加载成功
...
// 约定是：
    ...
    //- 第二个参数（和下一个参数，如果需要的话）用于成功的结果。
    //  此时 callback(null, result1, result2…) 就会被调用。
...
loadScript3('01.js', function(error, script) {

    if (error) {
        handleError(error);
    } else {
        // ...
        loadScript3('01.1.js', function(error, script) {
            if (error) {
                handleError(error);
            } else {
                // ...
                loadScript3('01.2.js', function(error, script) {
                    if (error) {
                        handleError(error);
                    } else {
                        // ...加载完所有脚本后继续 (*)
...
// 在上面这段代码中：
    //- 我们加载 01.js，如果没有发生错误。
    //- 我们加载 01.1.js，如果没有发生错误。
    //- 我们加载 01.2.js，如果没有发生错误 — 做其他操作 (*)。

// 如果调用嵌套的增加，代码层次变得更深，维护难度也随之增加，
// 尤其是我们使用的是可能包含了很多循环和条件语句的真实代码，而不是例子中的 ...。



// 可以通过使每个行为都成为一个独立的函数来尝试减轻这种问题，如下所示：
function loadScript3(src, callback) {
    let script = document.createElement('script');
    script.src = src;
...
// 此外，名为 step* 的函数都是一次性使用的，创建它们就是为了避免“厄运金字塔”。

// 没有人会在行为链之外重用它们。因此，这里的命名空间有点混乱。

// 我们希望还有更好的方法。有其他方法可以避免此类金字塔。最好的方法之一就是 “promise”
