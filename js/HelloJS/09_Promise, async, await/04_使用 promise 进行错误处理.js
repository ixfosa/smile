...
// Promise 链在错误（error）处理中十分强大。当一个 promise 被 reject 时，
// 控制权将移交至最近的 rejection 处理程序（handler）。这在实际开发中非常方便。
...
// 通常情况下，这样的 .catch 根本不会被触发。
// 但是如果上述任意一个 promise 被 reject（网络问题或者无效的 json 或其他），.catch 就会捕获它。
...
// 这不仅仅发生在 executor 函数中，同样也发生在其 handler 中。
// 如果我们在 .then 处理程序（handler）中 throw，这意味着 promise 被 rejected，
// 因此控制权移交至最近的 error 处理程序（handler）。
...
// 再次抛出（Rethrowing）
    // 链尾端的 .catch 的表现有点像 try..catch。
    // 可能有许多个 .then 处理程序（handler），然后在尾端使用一个 .catch 处理上面的所有 error。

    // 在常规的 try..catch 中，我们可以分析错误（error），如果我们无法处理它，可以将其再次抛出。
    // 对于 promise 来说，这也是可以的。

    // .catch 中 throw，那么控制权就会被移交到下一个最近的 error 处理程序（handler）。
...
.then(() => console.log("hahahaa")); // hahahaa

// 这里 .catch 块正常完成。所以下一个成功的 .then 处理程序（handler）就会被调用。


// 在下面的例子中，可以看到 .catch 的另一种情况。(*) 行的处理程序（handler）捕获了 error，
// 但无法处理它（例如，它只知道如何处理 URIError），所以它将其再次抛出：
new Promise((resolve, reject) => {
...
/*
    new Promise(function() {
        noSuchFunction(); // 这里出现 error（没有这个函数） ReferenceError: noSuchFunction is not defined
    })
    .then(() => {
...
// 如果出现 error，promise 的状态将变为 “rejected”，然后执行应该跳转至最近的 rejection 处理程序（handler）。
// 但是上面这个例子中并没有这样的处理程序（handler）。因此 error 会“卡住（stuck）”。没有代码来处理它。
...
// 如果出现了一个 error，并且在这儿没有 .catch，那么 unhandledrejection 处理程序（handler）就会被触发，
// 并获取具有 error 相关信息的 event 对象，所以我们就能做一些后续处理了。
...
// 当请求无法发出时，fetch reject 会返回 promise。
// 例如，远程服务器无法访问，或者 URL 异常。但是如果远程服务器返回响应错误 404，甚至是错误 500，
// 这些都被认为是合法的响应。

// 如果在 (*) 行，服务器返回一个错误 500 的非 JSON（non-JSON）页面该怎么办？
...
// 到目前为止，代码试图以 JSON 格式加载响应数据，但无论如何都会因为语法错误而失败。
// 你可以通过执行上述例子来查看相关信息，因为文件 no-such-user.json 不存在。
...
// 1. 为 HTTP 错误创建一个自定义类用于区分 HTTP 错误和其他类型错误。
//    此外，新的类有一个 constructor，它接受 response 对象，并将其保存到 error 中。因此，
//    错误处理（error-handling）代码就能够获得响应数据了。

// 2. 然后我们将请求（requesting）和错误处理代码包装进一个函数，
//    它能够 fetch url 并 将所有状态码不是 200 视为错误。这很方便，因为我们通常需要这样的逻辑。
...
// 请注意：这里的 .catch 会捕获所有错误，但是它仅仅“知道如何处理” HttpError 404。
// 在那种特殊情况下，它意味着没有这样的用户，而 .catch 仅仅在这种情况下重试。
