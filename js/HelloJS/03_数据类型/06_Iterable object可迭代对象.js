...
//- 如果从技术上讲，对象不是数组，而是表示某物的集合（列表，集合），
//- for..of 是一个能够遍历它的很好的语法，因此，让我们来看看如何使其发挥作用。
...
// 我们需要为对象添加一个名为 Symbol.iterator 的方法（一个专门用于使对象可迭代的内置 symbol）。
...
// 4. next() 方法返回的结果的格式必须是 {done: Boolean, value: any}，当 done=true 时，
//    表示迭代结束，否则 value 是下一个值。
...
// 请注意可迭代对象的核心功能：关注点分离。
    //- range 自身没有 next() 方法。
    //- 相反，是通过调用 range[Symbol.iterator]() 创建了另一个对象，即所谓的“迭代器”对象，
    //  并且它的 next 会为迭代生成值。
...
// 现在 range[Symbol.iterator]() 返回的是 range 对象自身：
    //- 它包括了必需的 next() 方法，并通过 this.current 记忆了当前的迭代进程。
    //- 这样更短，对吗？是的。有时这样也可以。

// 但缺点是，现在不可能同时在对象上运行两个 for..of 循环了：
    //- 它们将共享迭代状态，因为只有一个迭代器，即对象本身。但是两个并行的 for..of 是很罕见的，即使在异步情况下。
...
console.log(arr1.pop()); // World（pop 方法有效） World
console.log(arrayLike1); // { '0': 'Hello', '1': 'World', length: 2 }

// 在 (*) 行的 Array.from 方法接受对象，检查它是一个可迭代对象或类数组对象，然后创建一个 新数组，
// 并将该对象的所有元素复制到这个新数组。

// 如果是可迭代对象，也是同样：
// 假设 range 来自上文的例子中
let arr2 = Array.from(range2);
console.log(arr2); // [ 1, 2, 3, 4, 5 ] （数组的 toString 转化方法生效）
...
// 例如：
// 求每个数的平方
arr2 = Array.from(range2, num => num * num);
...
// 与 str.split 方法不同，它依赖于字符串的可迭代特性。因此，就像 for..of 一样，
// 可以正确地处理代理对（surrogate pair）。（译注：代理对也就是 UTF-16 扩展字符。）

// 技术上来讲，它和下面这段代码做的是相同的事：

let str2 = '𝒳😂';

chars = []; // Array.from 内部执行相同的循环
...
// 但 Array.from 精简很多。
// 我们甚至可以基于 Array.from 创建代理感知（surrogate-aware）的slice 方法
// （译注：也就是能够处理 UTF-16 扩展字符的 slice 方法）：
...
console.log( str3.slice(1, 3) ); // 乱码（两个不同 UTF-16 扩展字符碎片拼接的结果）
...
// 总结
    ...
    // 如果我们仔细研究一下规范 —— 就会发现大多数内建方法都假设它们需要处理的是可迭代对象或者类数组对象，
    // 而不是“真正的”数组，因为这样抽象度更高。

    // Array.from(obj[, mapFn, thisArg]) 将可迭代对象或类数组对象 obj 转化为真正的数组 Array，
    // 然后我们就可以对它应用数组的方法。可选参数 mapFn 和 thisArg 允许我们将函数应用到每个元素。
