...
// 数组可以存储任何类型的元素。
let arr = ['Apple', { name: 'Long' }, true, function() { console.log('hello'); } ]
...
// 队列（queue）是最常见的使用数组的方法之一。这表示支持两个操作的一个有序元素的集合：
    //- push 在末端添加一个元素.
...
console.log( fruits.pop() ); // 移除 "Lemon" 
...
// shift
    //- 取出数组的第一个元素并返回它：
console.log( fruits.shift() ); // 弹出 Apple
...
// 内部
    ...
    //- 它们扩展了对象，提供了特殊的方法来处理有序的数据集合以及 length 属性。
    //- 但从本质上讲，它仍然是一个对象。
...
p.push("zhong"); // 通过引用修改数组

console.log(person); // [ 'fo', 'long', 'zhong' ]



// 数组真正特殊的是它们的内部实现。JavaScript 引擎尝试把这些元素一个接一个地存储在连续的内存区域，
// 就像本章的插图显示的一样，而且还有一些其它的优化，以使数组运行得非常快。
...
let animal = []; // 创建一个数组
animal[66] = "dog";
animal.name = "大黑";  // 创建一个具有任意名称的属性

// 这是可以的，因为数组是基于对象的。我们可以给它们添加任何属性。

// 但是 Javascript 引擎会发现，我们在像使用常规对象一样使用数组，那么针对数组的优化就不再适用了，
// 然后对应的优化就会被关闭，这些优化所带来的优势也就荡然无存了。

// 数组误用的几种方式:
    //- 添加一个非数字的属性，比如 arr.test = 5。
    //- 制造空洞，比如：添加 arr[0]，然后添加 arr[1000] (它们中间什么都没有)。
    //- 以倒序填充数组，比如 arr[1000]，arr[999] 等等。
    //- 请将数组视为作用于 有序数据 的特殊结构。它们为此提供了特殊的方法。
        // 数组在 JavaScript 引擎内部是经过特殊调整的，使得更好地作用于连续的有序数据，
        // 所以请以正确的方式使用数组。如果你需要任意键值，那很有可能实际上你需要的是常规对象 {}。
...
// 那 push/pop 是什么样的呢？
    //-它们不需要移动任何东西。如果从末端移除一个元素，pop 方法只需要清理索引值并缩短 length 就可以了。
...
// 遍历数组最古老的方式就是 for 循环：
let persons = ["fo", "long", "zhong"];;
for (let i = 0; i < persons.length; i++) {
    console.log(persons[i]);  // "fo", "long", "zhong"
}
...
// 但这其实是一个很不好的想法。会有一些潜在问题存在：
    ...
    // 当然即使是这样也依然非常快。只有在遇到瓶颈时可能会有问题。但是我们仍然应该了解这其中的不同。
...
let fruits01 = [];
fruits01[123] = "Apple";
console.log( fruits01.length ); // 124
...
console.log( arr1[3] ); // undefined：被截断的那些数值并没有回来
...
// 让我们看看如何搬起石头砸自己的脚:
let arr3 = new Array(2); // 会创建一个 [2] 的数组吗？

console.log( arr3[0] ); // undefined！没有元素。
...
// 数组没有 Symbol.toPrimitive，也没有 valueOf，
// 它们只能执行 toString 进行转换，所以这里 [] 就变成了一个空字符串，[1] 变成了 "1"，[1,2] 变成了 "1,2"。
...
// 在这里的两个例子中，我们将原始类型和数组对象进行比较。
// 因此，数组 [] 被转换为原始类型以进行比较，被转换成了一个空字符串 ''。
// 然后，接下来的比较就是原始类型之间的比较, ''再转成number类型
...
// 比较数组时，不要使用 == 运算符（当然也不要使用 > 和 < 等运算符），因为它们不会对数组进行特殊处理。
// 它们通常会像处理任意对象那样处理数组，这通常不是我们想要的。
